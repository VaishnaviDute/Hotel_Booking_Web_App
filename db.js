...
function init() {
    db.serialize(() => {
        ...
        // Initial User
        db.get("SELECT id FROM users WHERE email = 'admin@hotel.com'", (err, row) => {
            if (!row) {
                db.run(`INSERT INTO users (email, name, password, phone, role) VALUES ('admin@hotel.com', 'Admin User', 'admin123', '1234567890', 'admin')`);
            }
        });
