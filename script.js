...
function setDefaultDates() {
    ...
    document.getElementById('m-checkin').min = todayStr;
    document.getElementById('checkout').min = tomorrowStr;
    document.getElementById('m-checkout').min = tomorrowStr;

    document.getElementById('checkin').value = tomorrowStr;
    document.getElementById('checkout').value = fmt(dayAfter);
    document.getElementById('m-checkin').value = tomorrowStr;
...
function bindSummaryUpdates() {
    ...
    // Enforce check-out is >= check-in
    const enforceMinCheckout = (e) => {
        const ciDate = e.target.value;
        if (!ciDate) return;

        // Set min value for checkouts
        document.getElementById('checkout').min = ciDate;
        document.getElementById('m-checkout').min = ciDate;

        // Auto-correct checkout if it's earlier than the new check-in
        const coField = document.getElementById('checkout');
        const mCoField = document.getElementById('m-checkout');
        
        if (coField.value && coField.value < ciDate) {
            coField.value = ciDate;
        }
        if (mCoField.value && mCoField.value < ciDate) {
            mCoField.value = ciDate;
        }
        updateSummary();
    };

    document.getElementById('checkin').addEventListener('change', enforceMinCheckout);
    document.getElementById('m-checkin').addEventListener('change', enforceMinCheckout);
}
...
function goStep(n) {
    ...
    if (wrap) {
        if (n >= 2 && n <= 3) wrap.classList.add('show-summary');
        else wrap.classList.remove('show-summary');
    }
    const sidebar = document.getElementById('bookingSidebar');
    if (sidebar) {
        if (n >= 2 && n <= 3) sidebar.style.display = 'block';
    ...
    try {
        ...
        if (usersBody) {
            usersBody.innerHTML = users.map(u => `
                <tr>
                    <td style="color:var(--gold); font-family:'Playfair Display';">#${u.id}</td>
                    <td>${u.name}</td>
                    <td>${u.email}</td>
                    <td><span class="status-badge ${u.role === 'admin' ? 'confirmed' : 'pending'}">${u.role}</span></td>
                    <td>${new Date(u.created_at).toDateString()}</td>
                    <td>
                        ${u.role === 'admin' ? '<span style="color:var(--muted);font-size:11px;">Restricted</span>' : `<button class="btn btn-outline" style="padding:4px 8px; font-size:10px; border-color:var(--red); color:var(--red);" onclick="deleteUser(${u.id}, '${u.name}')">Delete</button>`}
                    </td>
                </tr>
            `).join('');
        }
...
function toggleChatbot() {
    const bot = document.getElementById('concierge');
